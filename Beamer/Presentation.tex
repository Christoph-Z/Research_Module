...
\frametitle{Notation}
\begin{itemize}
	\item Let $A$ be any set, then $|A|$ denotes the nmber of elements of $A$
	\item $\hat{y}=X\hat{\beta}=X(X^\prime X)^{-1} X^\prime y$
	\item $\hat{y}_\alpha=X_\alpha(X_\alpha^\prime X_\alpha)^{-1} X_\alpha^\prime y$
