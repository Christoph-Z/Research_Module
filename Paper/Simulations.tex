...
\\
For the different simulations we consider the four model selection methods: \textit{CV(1), MCCV}($n_\nu$) and since they are popular in praxis \textit{AIC}, \textit{BIC} as benchmark. Also note that choose $n_v=n-\lfloor n^{3/4}\rfloor$ as in \cite{shao}.
