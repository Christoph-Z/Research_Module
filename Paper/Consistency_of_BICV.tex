...
By choosing $b=O_p(n)$ we can show that $BICV(n_v)$ is asymptomatically correct, under the following conditions :\footnote{Proof of Theorem \ref{THM_Consistency_BICV} is given in Appendix \RM{1}}
...
\end{thm}
~\\~
The most important difference between $CV(1)$ and the $BICV$ is the choice of $n_\nu$. Assumption \ref{growth_rates_nv_BICV} implies that $n_\nu$ and $n$ should grow with the same rate to infinity. Otherwise we would end up,asymptotically, in the same case as in $CV(1)$.\\
...
\begin{align*}
\Gamma_{\alpha,n-n_\nu}=\sigma^2+\frac{1}{n-n_\nu}~d_\alpha\sigma^2+O\left(\frac{1}{n-n_\nu}\right)
\end{align*}
is flat, in $\alpha$, such that it is difficult to find the minimum. But also we have to make sure that $n_nu$ is not to large, since we need $n-n_\nu\to\infty$ such that the last term of above equation vanishes.
