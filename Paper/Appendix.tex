...
\begin{proof}[Proof of Theorem \ref{THM_Consistency_BICV}]~\\
	...
	\end{align*}
	To see that this bound is valid, note that we can bound  
	\begin{align*}
	0=\min_{z\in \mathbb{R}^n : \lVert z\rVert=1}z'P_\alpha'z 
	\le \min_{z\in \mathbb{R}^n : \lVert z\rVert=1, z_{s^c}=0}z'P_\alpha z
	= \min_{z\in \mathbb{R}^{n_v} : \lVert z\rVert=1}z'Q_{\alpha,s}z
	\end{align*}
	and analogously
	\begin{align*}
	\max_{z\in \mathbb{R}^{n_v} : \lVert z\rVert=1}z'Q_{\alpha,s}z
	= \max_{z\in \mathbb{R}^n : \lVert z\rVert=1, z_{s^c}=0}z'P_\alpha z
	\le \max_{z\in \mathbb{R}^n : \lVert z\rVert=1}z'P_\alpha'z =1.
	\end{align*}
	Hence the maximal eigenvalue of $(I_{n_v}-Q_{\alpha,s})$ is bounded by one or equivalently 
	\begin{align*}
