...
%HIER auch annehmen dass die x_i's deerministisch sind?
...
\end{align*}
with $x_i\in\mathbb{R}^p$ as the vector of covariates with the assumption that $p$ is fixed, $\beta\in\mathbb{R}^p$ as the true parameter vector and the residual $\varepsilon_i$ for the obseravtion $i$.
%WEIß nicht ob man das so sagen kann mit iid.:
We assume that the residuals are independent and identically distributed and $E[\varepsilon_i]=0$ such as $E[\varepsilon_i^2]=\sigma^2$. Now we can write the linear regression model in matrix notation as
\begin{align*}
	y=X\beta+\varepsilon
\end{align*}
with $X=[x_1,...,x_n]^\prime\in\mathbb{R}^{n\times p}$ as our adequate matrix with the assumption of full rank, $y\in\mathbb{R}^{n}$ and $\varepsilon\in\mathbb{R}^{n}$.
\\\\
...
\end{tabular}
\end{align*}
Where Category~\RM{1} contains all the wrong models and Category~\RM{2} contains all models which may be too large. If the components of $\beta$ in the initial model are acquainted, then $\mathcal{M}_\alpha$ can be assigned to one of these two categories.\\\\
The optimal model will be
\begin{align*}
\mathcal{M}_\star=\{\mathcal{M}_{\alpha_\star}|\mathcal{M}_{\alpha_\star}\in \text{Cat.~\RM{2}~and~} dim(\mathcal{M}_{\alpha_\star})\leq dim(\mathcal{M}_{\alpha})\text{~for~any~}\mathcal{M}_{\alpha}\in \text{Cat.~\RM{2}} \}
\end{align*}
...
\begin{lemma}
	...
	\begin{align}
		\liminf_{n\rightarrow\infty}\frac{\Gamma_{\alpha,n}}{\Gamma_{\gamma,n}}>1~~~\Leftrightarrow~~~\liminf_{n\rightarrow\infty}\Delta_{\alpha,n}>0 \text{~~~for~}\mathcal{M}_\alpha\in\text{Category}~\RM{1} \label{liminf_condition}
	\end{align}
\end{lemma}
